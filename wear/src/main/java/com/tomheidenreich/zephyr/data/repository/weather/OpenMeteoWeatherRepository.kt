    ...
    private suspend fun refreshIfNeeded() {
        ...
        private fun shouldRefresh(now: Instant = clock.instant()): Boolean {
            return when (val current = state.value) {
                RepositoryResult.Loading -> true
                is RepositoryResult.Error -> true
                is RepositoryResult.Data -> now >= current.value.validUntil || current.freshness == DataFreshness.STALE
            }
        }
    }
}
